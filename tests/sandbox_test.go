...
func TestSandboxSuite(t *testing.T) {
	...
	suite.Run(t, &SandboxTestSuite{mode: options.Pooled, poolSize: numEntities * 2}) // pool > entities
	suite.Run(t, &SandboxTestSuite{mode: options.Pooled, poolSize: numEntities / 2}) // pool < entities
	suite.Run(t, &SandboxTestSuite{mode: options.Compact, poolSize: 0})
}
...
func (suite *SandboxTestSuite) TestSandbox_SimplePooling() {
	...
	// Re-link all
	for index := range numEntities {
		assert.NotNil(suite.T(), suite.positionLinker.Link(entity.Id(index)), "Re-Link(%d) returned nil", index)
	}
	sandbox.Update(suite.sandbox)

	// Verify components are accessible after relink
	for index := range numEntities {
		assert.NotNil(suite.T(), suite.positionLinker.Get(entity.Id(index)), "Get(%d) returned nil", index)
	}
}
